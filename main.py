...
def main():
    ...
    optimizer = torch.optim.AdamW(params, 2.5e-4)
    if args.load:
    ...
    for i in range(args.iterations):
        for step in range(args.nsteps):
            ...
            buf.done[step] = torch.tensor(done)

            with torch.no_grad():
                p1_latent = main_model([ob[0, step] for ob in buf.obs])
                p2_latent = main_model([ob[1, step] for ob in buf.obs])
                p1_logit, buf.values[0, step] = pv_model(p1_latent, p2_latent)
                p2_logit, buf.values[1, step] = pv_model(p2_latent, p1_latent)
        ...
        print(f"{env.lines/count:.1f} lines, {env.atk/count:.1f} atk, {buf.rews.mean().cpu().numpy():.4f} rewards avg")
        with torch.no_grad():
            ...
            _, next_val1 = pv_model(p1_latent, p2_latent)
            _, next_val2 = pv_model(p2_latent, p1_latent)
        ...
        for i in range(args.nepoch):
            order = np.arange(args.nenvs*args.nsteps*2)
            ...
            for j in range(args.nminibatch):
                ...
                logits, new_values = pv_model(*latents)
                acs, log_prob, vals = map(lambda x: x[sli], flat_buf)
                ...
                optimizer.step()
                print(f"{pg_loss=:.4f} {v_loss=:.4f} {entropy_loss=:.4f}")
        torch.save({
