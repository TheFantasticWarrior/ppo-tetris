...
class Model(nn.Module):
    def __init__(self, ff_size=ff_size, d_model=d_model):
        ...
        self.rotation_embedding = nn.Linear(d_model, d_model*16)
        ...
        self.attn_bias = nn.ModuleList(
            [Bias_Layer(ff_size, seq_len, 1) for _ in range(7)] +
            [Bias_Layer(ff_size, seq_len, 7)])

        self.queue_linear = nn.ModuleList(
            [nn.Linear(ff_size, ff_size) for _ in range(8)])
        ...
        self.t_encoder = nn.Transformer(
            d_model=ff_size, nhead=8, dim_feedforward=ff_size*2,
            batch_first=True, num_encoder_layers=2, num_decoder_layers=0
        ).encoder
    ...
    def forward(self, x):
        ...
        pieces_with_rotations = self.rotation_embedding(
            pieces_processed).view(batch_size, -1, 16, self.d_model)
        ...
        def attn(state, pieces, i):
            ...
            v = self.v_linear[i](pieces).view(
                batch_size, -1, num_heads, d_head)
            bias = self.attn_bias[i](state).view(
        ...
        state = self.attn_norm[7](state+new_state)

        new_state = self.queue_linear[7](state)
        state = self.ff_norm[7](state+new_state)
        state = self.t_encoder(state)
...
class FinalModel(nn.Module):
    def __init__(self, ff_size=ff_size, d_model=d_model):
        ...
        self.attn3 = nn.MultiheadAttention(d_model, 8, batch_first=True)
        self.pos_embed = PositionalEncoding(d_model, max_len=50)
        self.t_encoder = nn.Transformer(
            d_model=d_model, nhead=8, dim_feedforward=ff_size,
            batch_first=True, num_encoder_layers=2, num_decoder_layers=0
        ).encoder
        self.linear = nn.Linear(50, 1)
        self.policy = nn.Linear(d_model, 10)
    ...
    def forward(self, x, y, retain, mem_only=False):
        ...
        x = self.pos_embed(x)
        x = self.t_encoder(x)
        seq_last = x.swapaxes(1, 2)
        memory = self.memory(seq_last).swapaxes(1, 2)
        if mem_only:
            return memory
        kv = self.linear(seq_last).swapaxes(1, 2)
        x, _ = self.attn3(kv, x, x)
